...
// This version may contain the sleep mode.
package main
...
type Sim struct {
	...
	ActCycLog    *etable.Table     `view:"no-inline" desc:"Keep track of the activation on cycle-level, log data"`
	SlpCycLog    *etable.Table     `view:"no-inline" desc:"Similarities between current and previous networks"` // added by DH.
	...
	SlpAvgSml  float64 `inactive:"+" desc:"the average correlation of the current state and previous state for all network layers"` //Added by DH
	...
	SlpCycPlot   *eplot.Plot2D    `view:"-" desc:"the sleep-cycle plot"`
...
func (ss *Sim) Config() {
	...
	ss.ConfigSlpCycLog(ss.SlpCycLog) // Added by DH
...
// TODO The SleepCycMon is a the func that keep tracks of the similarities of current and previous states.
// This will be used to set the SleepCyc into plus or minus phases.
// How? Don't know yet.

func (ss *Sim) SleepCycMon(en env.Env) {
...
func (ss *Sim) SleepCyc(WakeReplay bool) {
	...
	for cyc := 0; cyc < ss.MaxSlpCyc; cyc++ {
		if WakeReplay {
			// Need to set the parameters for WakeReplay here.
		} else {
			// Need to set the paramenters for SWS sleep here.
		}
...
func (ss *Sim) SleepTrial() {
	// ss.TrainEnv.Step() // Should it step or not?
	epc, _, chg := ss.TrainEnv.Counter(env.Epoch)
	if chg {
		// Need to update the View. See the TrainTrial for an example.
	}
...
// LogActCyc adds data from current trial to the TstTrlLog table.
// log always contains number of testing items
func (ss *Sim) LogActCyc(dt *etable.Table) {
	inLay := ss.Net.LayerByName("Input").(*leabra.Layer)
	blaNeInLay := ss.Net.LayerByName("Ne").(*leabra.Layer)
	blaPoInLay := ss.Net.LayerByName("Po").(*leabra.Layer)
	hid1Lay := ss.Net.LayerByName("Hidden1").(*leabra.Layer)
	...
	trl := ss.SleepEnv.Trial.Cur
	cyc := ss.SleepEnv.Cycle.Cur

	dt.SetCellFloat("Run", cyc, float64(ss.TrainEnv.Run.Cur))
	dt.SetCellFloat("Epoch", cyc, float64(epc))
	dt.SetCellFloat("Trial", cyc, float64(trl))
	dt.SetCellFloat("Cycle", cyc, float64(cyc))
	dt.SetCellString("TrialName", cyc, ss.SleepEnv.TrialName)

	dt.SetCellTensor("InAct", cyc, inLay.UnitValsTensor("Act"))
	dt.SetCellTensor("OutAct", cyc, outLay.UnitValsTensor("Act"))
	dt.SetCellTensor("Hidden1", cyc, hid1Lay.UnitValsTensor("Act"))
	dt.SetCellTensor("BlaNeInAct", cyc, blaNeInLay.UnitValsTensor("Act"))
	dt.SetCellTensor("BlaPoInAct", cyc, blaPoInLay.UnitValsTensor("Act"))
	dt.SetCellTensor("BlaNeOutAct", cyc, blaNeOutLay.UnitValsTensor("Act"))
	dt.SetCellTensor("BlaPoOutAct", cyc, blaPoOutLay.UnitValsTensor("Act"))

	// note: essential to use Go version of update when called from another goroutine
	ss.ActCycPlot.GoUpdate()
}

func (ss *Sim) ConfigActCycLog(dt *etable.Table) {
	inLay := ss.Net.LayerByName("Input").(*leabra.Layer)
	blaNeInLay := ss.Net.LayerByName("Ne").(*leabra.Layer)
	blaPoInLay := ss.Net.LayerByName("Po").(*leabra.Layer)
	outLay := ss.Net.LayerByName("Output").(*leabra.Layer)
	...
	dt.SetMetaData("name", "ActCycLog")
	dt.SetMetaData("desc", "Record of activity per sleep cycle")
	dt.SetMetaData("read-only", "true")
	dt.SetMetaData("precision", strconv.Itoa(LogPrec))

	nt := ss.SleepEnv.Table.Len() // number in view
	dt.SetFromSchema(etable.Schema{
		{"Run", etensor.INT64, nil, nil},
		{"Epoch", etensor.INT64, nil, nil},
		{"Trial", etensor.INT64, nil, nil},
		...
		{"TrialName", etensor.STRING, nil, nil},
		{"InAct", etensor.FLOAT64, inLay.Shp.Shp, nil},
		{"OutAct", etensor.FLOAT64, outLay.Shp.Shp, nil},
		{"Hidden1 Act", etensor.FLOAT64, hid1Lay.Shp.Shp, nil},
		{"BlaNeInAct", etensor.FLOAT64, blaNeInLay.Shp.Shp, nil},
		{"BlaPoInAct", etensor.FLOAT64, blaPoInLay.Shp.Shp, nil},
		{"BlaNeOutAct", etensor.FLOAT64, blaNeOutLay.Shp.Shp, nil},
		{"BlaPoOutAct", etensor.FLOAT64, blaPoOutLay.Shp.Shp, nil},
...
func (ss *Sim) ConfigActCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Leabra Summer Sleep Cycle Plot"
	plt.Params.XAxisCol = "Cycle"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
	plt.SetColParams("Run", false, true, 0, false, 0)
	...
	plt.SetColParams("InAct", false, true, 0, true, 1)
	plt.SetColParams("OutAct", false, true, 0, true, 1)
	plt.SetColParams("Hidden1", false, true, 0, true, 1)
	plt.SetColParams("BlaNeInAct", false, true, 0, true, 1)
	plt.SetColParams("BlaPoInAct", false, true, 0, true, 1)
	plt.SetColParams("BlaNeOutAct", false, true, 0, true, 1)
	plt.SetColParams("BlaPoOutAct", false, true, 0, true, 1)
...
// LogSlpCyc adds data from current run to the SlpCycLog table.
func (ss *Sim) LogSlpCyc(dt *etable.Table, cyc int) {
	...
	// Calculate them.
	dt.SetCellFloat("Cycle", cyc, float64(cyc))
	curAct, _ := ss.ActCycLog.CellTensorTry("InAct", cyc)
	preAct, _ := ss.ActCycLog.CellTensorTry("InAct", cyc-5)
	dt.SetCellFloat("In Sml", cyc, float64())
...
func (ss *Sim) ConfigSlpCycLog(dt *etable.Table) {
	...
	dt.SetFromSchema(etable.Schema{
		{"Cycle", etensor.INT64, nil, nil},
		{"In Sml", etensor.FLOAT64, nil, nil},
		{"BlaNeIn Sml", etensor.FLOAT64, nil, nil},
		{"BlaPoIn Sml", etensor.FLOAT64, nil, nil},
		{"Hid1 Sml", etensor.FLOAT64, nil, nil},
		{"Out Sml", etensor.FLOAT64, nil, nil},
		{"BlaNeOut Sml", etensor.FLOAT64, nil, nil},
		{"BlaPoOut Sml", etensor.FLOAT64, nil, nil},
	}, np)
}

func (ss *Sim) ConfigSlpCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	plt.Params.Title = "Summer Sleep model Sleep Cycle Plot"
	plt.Params.XAxisCol = "Cycle"
	plt.SetTable(dt)
	// order of params: on, fixMin, min, fixMax, max
