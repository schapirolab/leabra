...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	blaInLay := net.AddLayer2D("BlaIn", 3, 1, emer.Input)
	...
	blaOutLay := net.AddLayer2D("BlaOut", 3, 1, emer.Target)

	// use this to position layers relative to each other
	// default is Above, YAlign = Front, XAlign = Center
	hid2Lay.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Hidden1", YAlign: relpos.Front, Space: 2})
	blaInLay.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Input", YAlign: relpos.Front, Space: 2})
	blaOutLay.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "Output", YAlign: relpos.Front, Space: 2})

	net.ConnectLayers(inLay, hid1Lay, prjn.NewFull(), emer.Forward)
	net.ConnectLayers(blaInLay, hid1Lay, prjn.NewFull(), emer.Forward)
	net.ConnectLayers(hid1Lay, hid2Lay, prjn.NewFull(), emer.Forward)
	net.ConnectLayers(hid2Lay, outLay, prjn.NewFull(), emer.Forward)
	net.ConnectLayers(hid2Lay, blaOutLay, prjn.NewFull(), emer.Forward)

	// note: see emergent/prjn module for all the options on how to connect
	// NewFull returns a new prjn.Full connectivity pattern
	net.ConnectLayers(outLay, hid2Lay, prjn.NewFull(), emer.Back)
	net.ConnectLayers(blaOutLay, hid2Lay, prjn.NewFull(), emer.Back)
...
func (ss *Sim) ApplyInputs(en env.Env) {
	...
	blaInLay := ss.Net.LayerByName("BlaIn").(*leabra.Layer)
	outLay := ss.Net.LayerByName("Output").(*leabra.Layer)
	blaOutLay := ss.Net.LayerByName("BlaOut").(*leabra.Layer)

	inPats_In := en.State(inLay.Nm)
	inPats_Bla := en.State(blaInLay.Nm)
	if (inPats_In != nil) || (inPats_Bla != nil) {
		inLay.ApplyExt(inPats_In)
		blaInLay.ApplyExt(inPats_Bla)
	}
	outPats_Out := en.State(outLay.Nm)
	outPats_Bla := en.State(blaOutLay.Nm)
	if (inPats_In != nil) || (inPats_Bla != nil) {
		outLay.ApplyExt(outPats_Out)
		blaOutLay.ApplyExt(outPats_Bla)
	}
}
...
func (ss *Sim) ConfigPats() {
	...
	dt.SetFromSchema(etable.Schema{
		...
		{"BlaIn", etensor.FLOAT32, []int{3, 1}, []string{"Y", "X"}},
		{"Input", etensor.FLOAT32, []int{5, 5}, []string{"Y", "X"}},
		{"Output", etensor.FLOAT32, []int{5, 5}, []string{"Y", "X"}},
		{"BlaOut", etensor.FLOAT32, []int{3, 1}, []string{"Y", "X"}},
	}, 25)
...
func (ss *Sim) LogTrnEpc(dt *etable.Table) {
	...
	dt.SetCellFloat("BlaOut ActAvg", row, float64(blaOutLay.Pools[0].ActAvg.ActPAvgEff))
...
func (ss *Sim) ConfigTrnEpcLog(dt *etable.Table) {
	...
	dt.SetFromSchema(etable.Schema{
		...
		{"BlaOut ActAvg", etensor.FLOAT64, nil, nil},
	}, 0)
}
...
func (ss *Sim) ConfigTrnEpcPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	...
	plt.SetColParams("BlaOut ActAvg", false, true, 0, true, .5)
	return plt
}
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	...
	dt.SetCellFloat("BlaOut ActM.Avg", trl, float64(blaOutLay.Pools[0].ActM.Avg))

	dt.SetCellTensor("InAct", trl, inLay.UnitValsTensor("Act"))
	dt.SetCellTensor("BlaInAct", trl, blaInLay.UnitValsTensor("Act"))
	dt.SetCellTensor("OutActM", trl, outLay.UnitValsTensor("ActM"))
	dt.SetCellTensor("OutActP", trl, outLay.UnitValsTensor("ActP"))
	dt.SetCellTensor("BlaOutAct", trl, blaOutLay.UnitValsTensor("Act"))
...
func (ss *Sim) ConfigTstTrlLog(dt *etable.Table) {
	...
	dt.SetFromSchema(etable.Schema{
		...
		{"BlaOut ActM.Avg", etensor.FLOAT64, nil, nil},
		{"InAct", etensor.FLOAT64, inLay.Shp.Shp, nil},
		{"BlaInAct", etensor.FLOAT64, blaInLay.Shp.Shp, nil},
		{"BlaOutAct", etensor.FLOAT64, blaOutLay.Shp.Shp, nil},
...
func (ss *Sim) ConfigTstTrlPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	...
	plt.SetColParams("BlaOut ActM.Avg", true, true, 0, true, .5)

	plt.SetColParams("InAct", false, true, 0, true, 1)
	plt.SetColParams("BlaInAct", false, true, 0, true, 1)
	plt.SetColParams("OutActM", false, true, 0, true, 1)
	plt.SetColParams("OutActP", false, true, 0, true, 1)
	plt.SetColParams("BlaOutAct", false, true, 0, true, 1)
	return plt
}
...
func (ss *Sim) LogTstEpc(dt *etable.Table) {
	...
	split.Agg(allsp, "BlaInAct", agg.AggMean)
	split.Agg(allsp, "OutActM", agg.AggMean)
	split.Agg(allsp, "OutActP", agg.AggMean)
	split.Agg(allsp, "BlaOutAct", agg.AggMean)
...
func (ss *Sim) LogTstCyc(dt *etable.Table, cyc int) {
	...
	dt.SetCellFloat("BlaOut Ge.Avg", cyc, float64(blaOutLay.Pools[0].Ge.Avg))
	...
	dt.SetCellFloat("BlaOut Act.Avg", cyc, float64(blaOutLay.Pools[0].Act.Avg))
...
func (ss *Sim) ConfigTstCycPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	...
	plt.SetColParams("BlaOut Ge.Avg", true, true, 0, true, .5)
	...
	plt.SetColParams("BlaOut Act.Avg", true, true, 0, true, .5)
	return plt
}
